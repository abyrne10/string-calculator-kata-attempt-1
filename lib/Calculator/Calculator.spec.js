...
describe('Calculator module', () => {
    describe('"add"', () => {
        ...
        it('should throw an error \'negatives not allowed\' if a negative number is passed', () => {
            expect(() => { Calculator.add('1,2,-3,4'); }).to.throw(Error, 'negatives not allowed').with.property('negatives', JSON.stringify([-3]));
        });
        it('should throw an error \'negatives not allowed\' if a negative number is passed', () => {
            expect(() => { Calculator.add('// \n-1 2 3 -4 5 -6'); }).to.throw(Error, 'negatives not allowed').with.property('negatives', JSON.stringify([-1, -4, -6]));
        });
    });
});
