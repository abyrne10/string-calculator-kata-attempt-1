...
function add(numbers) {
    ...
    if (customDelimiterSpecified) {
        ...
        const strings = getStringsWithCustomDelimiter(numbers, delimiter);
        result = getSumOfNumbersInArrayOfStrings(strings);
    } else {
        const strings = getStringsWithStandardDelimiters(numbers);
        result = getSumOfNumbersInArrayOfStrings(strings);
    }
    return result;
}
...
function getCustomDelimiter(numbers) {
    ...
    const firstLine = splitOnNewline[0];
    const delimiterStarts = customDelimiterIndicator.length;
    const delimiterEnds = firstLine.length;
    const delimiter = firstLine.slice(delimiterStarts, delimiterEnds);
    return delimiter;
}
