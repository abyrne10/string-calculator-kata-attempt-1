...
function add(numbers) {
    ...
    const strings = splitIntoStrings(numbers);
    const sum = getSum(strings);
    return sum;
}

function splitIntoStrings(numbers) {
    checkArgIsOfType(numbers, Type.string);
    let strings;
    const customDelimiterSpecified = checkForCustomDelimiter(numbers);
    if (customDelimiterSpecified) {
        strings = getStringsWithCustomDelimiter(numbers);
    } else {
...
function checkForNegatives(ints) {
    ...
    if (negatives) {
        const err = new Error();
        err.message = `negatives not allowed: ${JSON.stringify(negatives)}`;
        throw err;
    }
}
...
function getStringsWithCustomDelimiter(numbers) {
    checkArgIsOfType(numbers, Type.string);
    const delimiter = getCustomDelimiter(numbers);
    const stringToSum = getStringToSum(numbers);
    const splitOnDelimiter = stringToSum.split(delimiter);
    return splitOnDelimiter;
}

function getStringToSum(numbers) {
    checkArgIsOfType(numbers, Type.string);
